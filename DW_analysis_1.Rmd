...
```{r, echo=FALSE}
plot_1 <-USA %>% ggplot() + geom_line(aes(x=Reported,y=Cases,col="USA")) +
    geom_line(data=JHU,aes(x=date,y=DC,col="John Hopkins")) +
    geom_line(data=NYT,aes(x=date,y=DC,col="NY Times")) +
  labs(title="USA/John Hopkins/NY Times: Daily Cases for  the US")

plot_2 <- USA %>% ggplot() + geom_line(aes(x=Reported,y=Deaths,col="USA")) +
    geom_line(data=JHU,aes(x=date,y=DD,col="John Hopkins")) +
    geom_line(data=NYT,aes(x=date,y=DD,col="NY Times")) +
  labs(title="USA/John Hopkins/NY Times: Daily Death for  the US")
ggplotly(plot_1)
ggplotly(plot_2)
```
...
```{r}
AA<- USA %>% filter(Reported >="2020-06-01" )
