...
```{r, echo=FALSE}
plot_1 <-ECDC %>% ggplot() + geom_line(aes(x=Reported,y=Cases,col="ECDC")) +
...
plot_2 <- ECDC %>% ggplot() + geom_line(aes(x=Reported,y=Deaths,col="ECDC")) +
...
```{r,echo=FALSE}
A <- ECDC %>% filter(Reported >="2020-04-01" & Reported <="2020-04-30")
...
A %>% ggplot() + geom_line(aes(x=Reported,y=Cases,col="ECDC")) +
  ...
  labs(title="First Spike in U.S. Daily Cases (Apil 2020")
...
### Current Spike in U.S. Daily Cases and Deaths: June 1, 2020 - Present

```{r}
AA<- ECDC %>% filter(Reported >="2020-06-01" )
BA<- JHU %>% filter(date >="2020-6-01" )
CA <- NYT %>% filter(date >="2020-06-01")
```
```{r}
AA %>% ggplot() + geom_line(aes(x=Reported,y=Cases,col="ECDC")) +
    geom_line(data=BA,aes(x=date,y=DC,col="John Hopkins")) +
    geom_line(data=CA,aes(x=date,y=DC,col="NY Times")) +
  labs(title="Current  Spike in Cases: June 1, 2020 - Present")

AA %>% ggplot() + geom_line(aes(x=Reported,y=Deaths,col="ECDC")) +
    geom_line(data=B,aes(x=date,y=DD,col="John Hopkins")) +
    geom_line(data=C,aes(x=date,y=DD,col="NY Times")) +
  labs(title="Current Spike in Deaths: June 1, 2020 - Present")
```
